...
// Mutate calls f(ctx, m).
func (f ProductVideoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ProductVideoMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ProductVideoMutation", m)
}

// The UserVideoFunc type is an adapter to allow the use of ordinary
// function as UserVideo mutator.
type UserVideoFunc func(context.Context, *ent.UserVideoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserVideoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserVideoMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserVideoMutation", m)
}
